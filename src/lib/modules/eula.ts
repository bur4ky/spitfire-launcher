...
export class EULA {
  ...
  static async check(account: AccountData) {
    const response = await AuthSession.ky(account, eulaService).get<EULACheckData>(`account/${account.accountId}`);
  ...
  static accept(account: AccountData, version: number) {
    return AuthSession.ky(account, eulaService).post(`version/${version}/${account.accountId}/accept`);
  }
}
